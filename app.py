...
st.sidebar.markdown("""<h1 style='text-align: center;color:  #0e76a8;'><a style='text-align: center;color:  #0e76a8;' href="https://www.linkedin.com/in/harish-natarajan-82a4b418b/" target="_blank">Linkedin Profile</a></h1>""", unsafe_allow_html=True)
...
st.sidebar.markdown("""<style>body {background-color: #2C3454; background-image: url('https://i2.wp.com/highland-music.com/wp-content/uploads/2016/04/Blue-Background-Music-Headphone-Wallpaper-Picture-HD-Free-298292334-e1459743028815.png?ssl=1'); color:white;}</style><body></body>""", unsafe_allow_html=True)
st.markdown("""<h1 style='text-align: center; color: white;font-size:60px;margin-top:-50px;'>AUDIO CLASSIFIER</h1><h1 style='text-align: center; color: white;font-size:30px;margin-top:-30px;'>Using Machine Learning</h1>""",unsafe_allow_html=True)
...
if file is not None:
    ...
    if rad == "Predict":
        if st.button("Classify Audio"):
            ...
            st.markdown(f"""<h1 style='color:yellow;'>Prediction : <span style='color:white;'>{targets[0][0]}</span></h1>""", unsafe_allow_html=True)
